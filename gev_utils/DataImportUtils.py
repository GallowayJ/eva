...
class WebScraper:
    """Scrapes met station data and parses data into a usable format.
        ...
        Attributes:
            ...
            self.station_links (dict): A dictionary where keys are station names and values are links to station data.
            self.station_dict (dict): A dictionary where keys are station names and values are a list of data and metadata.
            self.nonstandard_list (list): A list of stations where the format of the metadata does not permit automatic parsing.
    """
    def __init__(self, base_url):
        ...
        Returns:
            An instance of webscraper. NB WebScraper.get_data() must be called for
    ...
    def get_stations_and_links(self):
        """Scrapes base_url for met station names and met station links. Updates
        self.station_links and creates a dictionary where keys are met station names and
        values are met station links.
    ...
    def get_coordinates(self):
        """Attempts to extract coordinates from station metadata. If this fails,
        add the station name to self.nonstandard_list.
    ...
    def get_data(self):
        """Scrapes met station data (measurements) and metadata (coordinates, whether a station
        was relocated) from self.base_url. Modifies self.station_dict values to contain a list
        of the station's link, a pandas.DataFrame of station measurements and a list metadata.

        Returns:
            self.station_dict (dict): A dict where keys are station names and is a list of station data
            and metadata.
        """
...
class DataMunger:
    """Carries out data munging of self.data_dict. Drops unneeded data columns, removes header units,
        strips characters from numeric fields so that correct dtypes can be assigned, extracts station
        coordinates from station metadata, creates a datatime index and contains a method to return an
        extreme value dict.

            Attributes:
                self.data_dict (dict): A dictionary where keys are station names and values are lists of station data and metadata.
                self.station_coords_list (list): A list of latitude and longitude extracted from station metadata.
    ...
    def drop_unneeded_cols(self, targ_col=["Rainfall"]):
        """Drops all columns apart from 'Year', 'Month' and targ_column.
           ...
           Returns:
                self.data_dict (dict): Modified data_dict consisting of columns in targ_col and 'Year' and 'Month' columns.
    ...
    def strip_asterices(self):
        """Iterates through self.data_dict, DataFrames containing asterices are removed.
            Each DataFrame in self.data_dict is modified in place.
    ...
    def get_station_coords(self):
        """Iterates through self.data_dict, latitude and longitude is extracted from
            metadata and appended to self.station_coords_list.
    ...
    def assign_correct_dtypes(self):
        """Iterates through self.data_dict. Rows containing missing observations are dropped,
            and the following dtypes are assigned: Year (Int64), Month (Int64), Rainfall (float32).
    ...
    def create_dt_index(self):
        """Iterates through self.data_dict, Year and Month columns are used to create a
            datetime intex which is assigned to a new column, 'Date' which is set to the
            index of the DataFrame. Year and Month columns are then dropped.
    ...
    def get_extreme_dict(self, data_dict):
        """Iterates through a data_dict and resamples by year, returning the maxima.
            It returns a dictionary containing the resampled DataFrames.

            Args:
                data_dict (dict): A dictionary where the keys are station names and values
                                    are DataFrames. DataFrames must have a datetime index.

            Returns:
                maxima_dict (dict): A dictionary where the keys are station names and the
                                        values are DataFrames containing the annual maxima.
        """
    ...
    def tidy_data(self):
        """Carries out data munging by calling self.drop_unneeded_cols,
            self.remove_units_header, self.strip_asterices, self.assign_correct_dtypes,
            self.create_dt_index, self.get_station_coords.
    ...
    def save_data(self, directory, **kwargs):
        """Iterates through self.data_dict and saves each DataFrame into 'directory' as
        a .csv named by the dictionary key (station name).
...
class GeoMunger:
    """Handles parsing, unifification and tidying of spatial data and supporting
    information (house price data and modelling results).

        Attributes:
            self.house_price (pandas.DataFrame): A pandas.DataFrame of average house prices
            self.station_coords (pandas.DataFrame): A pandas.DataFrame of station latitude and longitude.
            self.metrics_df (pandas.DataFrame): A pandas.DataFrame of predictions, error bounds and supporting data.
            self.unified_gdf (geopandas.GeoDataFrame): A geopandas.GeoDataFrame containing statistical data and geospatial data.
            self.aoi_outline (geopandas.GeoDataFrame): A geopandas.GeoDataFrame of the outline of the area of interest (aoi).
    """
    ...
    def __init__(self, gdf_house_price, gdf_station_coords, aoi_outline=None):
        """Constructs an instance of GeoMunger.
            ...
            Args:
                gdf_house_price (GeoDataFrame): A GeoDataFrame containing average house prices for each region.
                gdf_station_coords (GeoDataFrame): A GeoDataFrame containing the coordinates (lat, lon) of each station.

            Kwargs:
                aoi_outline (GeoDataFrame): A GeoDataFrame delinating the outline of the study area. Used for visualisation.
    ...
    def extract_station_metrics(self, gev_dict):
        """Extracts mean predicted return intensity and the 95% HDI from an instance
        of gev_dict and the corresponding return periods.

        Args:
            gev_dict (dict): An instance of gev_dict generated by fitting extreme events to the generalized extreme value
                            distribution using PyMC. NB gev_dict is generated by the accompaning Jupyter Notebook.

        Returns:
            station_metrics (pd.DataFrame): A pandas DataFrame containing only the information needed for carrying out analysis in the
                                accompaning Jupyter noteobook.
        """
    ...
    def get_return_period_results(self, station_name, df, return_period_yrs=20):
        ...
        Args:
            ...
            df (pandas.DataFrame): A pandas.DataFrame containing the results of the posterior preditive containing
                                 the return period, return intensity and HDI (95% is used in this example).

        Kwargs:
            return_period_yrs (int): The return period of interest, default 20.

        Returns:
            df (pandas.DataFrame): A pandas.DataFrame containing the return intensity, lower and upper
                                intervals for `return_period_yrs`.
        """
    ...
    def get_metrics_df(self, gev_dict):
        """Updates self.metric_df with mean prediction and lower and upper bound of the HDI (by default 95%) by
            extracting this information from `gev_dict`.
    ...
    def add_metrics_to_stations(self):
        """Merges `self.station_coords` and `self.metrics_df` on station name with the result
        set to `self.metrics_df`.
    ...
    def convert_crs(self, crs="EPSG:27700"):
        """Converts the crs of `self.house_price` and `self.station_coords` to `crs`.
            ...
            Kwargs:
                crs (str): A valid crs string to be passed to geopandas.GeoDataFrame.to_crs(), default is `EPSG:27700`, recommended for the UK.
    ...
    def join_house_prices(self, cols_to_keep=None):
        """Joins `self.house_price` and `self.station_coords` using geopandas.sjoin_nearest() and
            sets the result to `self.unified_gdf`. The result is that met station will be assigned to their
            geographically nearest house price region. Also drops unneeded columns, a list of column names
            can be specified using `cols_to_keep` to change which columns are kept.

                Kwargs:
                    cols_to_keep (list-like): A list of columns names to be kept in the resulting GeoDataFrame.
    ...
    def remove_nans(self):
        """Removes rows from `self.unified_gdf` which the column 'Average_Price' is NA and resets the index of `self.unified_gdf`.
    ...
    def minmax_normalizer(
        ...
        normalized values.
            ...
            Kwargs:
                risk_var (str): The name of the column to be normalized, default 'Return intensity'.
                hdi_cols (list): A list of length 2, where each element is a string of the name of the columns containing
                                the lower and upper HDI, respectively. Default value is `['Lower', 'Upper'].
                upper_bound (float): Specify the largest value, effectively control the scale of the normalizer.

            Returns:
                adj_multiplier (pandas.DataFrame): A pandas.DataFrame containing normalized values.
        """
    ...
    def get_risk_adjusted_hp(self, df, adjustment_df, threshold=160):
        ...
        above threshold. adjustment_df is a pandas.DataFrame of normalized rainfall.
            ...
            Kwargs:
                threshold (int): The value that 'Return intensity' should exceed for risk-adjustment to take place, default value 160 (mm).

            Returns:
                df (pandas.DataFrame): A pandas.DataFrame which is `df` with additional columns containing
                                        risk-adjustment in addition to a column contain the values used to
                                        peform risk-adjustment.
    ...
    def get_error_bounds(
        ...
        interval to the risk-adjusted house prices
            ...
            Args:
                df (pandas.DataFrame): A pandas.DataFrame containing containing the
                                        95% HDI and uses the ratio of the lower and
                                        upper bounds to calculate error for
                                        risk-adjusted house prices.

            Kwargs:
                risk_var (str): The name of the column containing the risk variable in `df`.
                targ_col (str): The name of the column continaing the risk-adjusted values in `df`.
                hdi_cols (list): A list of length 2 containing the names in the HDI columns in `df`
                                order must be lower bound followed by upper bound.
        """
    ...
    def get_risk_adj_df(
        ...
        place (in [mm] rainfall).
            ...
            Kwargs:
                df (pandas.DataFrame): A pandas.DataFrame containing the risk varible predictions, HDI bounds and house prices.
                risk_var (str): The name of the risk variable in `df`, default value 'Return intensity'.
                adj_col (str): The name of the column containing values to be risk-adjusted in `df`, default 'Average_Price'.
                risk_weight (float): The weighting that risk_var should carry, larger values give a greater weighting. This value
                                    is passed to self.minmax_normalizer(). Default 0.5.
                risk_threshold (int): The value that should be exceeded for risk-adjustment to occur, default 160 (mm rainfall).

            Returns:
                df (pandas.DataFrame): A pandas.DataFrame containing risk-adjusted values and HDI bounds appendeded to `df`.
        """
        if df is None:
